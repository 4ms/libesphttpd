...
//#define DEMO_MODE
	...
	#define time_after(a, b) (typecheck(unsigned int, a) && typecheck(unsigned int, b) && ((long)((b) - (a)) < 0))
...
const char *state_names[] = {
	"Failed", "Connected", "Idle", "Update", "WPS Start", "WPS Active", "Connecting", "Fall Back"};
...
esp_err_t initCgiWifi(void) {
	...
	scan_timer = xTimerCreate("Scan_Timer", SCAN_TIMEOUT, pdFALSE, NULL, handle_scan_timer);
	if (scan_timer == NULL) {
		ESP_LOGE(TAG, "[%s] Failed to create scan timeout timer", __FUNCTION__);
		result = ESP_ERR_NO_MEM;
		goto err_out;
	}

	config_timer = xTimerCreate("Config_Timer", CFG_TICKS, pdFALSE, NULL, handle_config_timer);
	if (config_timer == NULL) {
		ESP_LOGE(TAG, "[%s] Failed to create config validation timer", __FUNCTION__);
		result = ESP_ERR_NO_MEM;
		goto err_out;
	}
...
static void wifi_scan_done(system_event_t *event) {
	...
	if (atomic_load(&scan_in_progress) == false) {
		...
		\* is triggering scans.                                      */
		ESP_LOGE(TAG, "[%s] Received unsolicited scan done event.", __FUNCTION__);
		return;
	}

	if (event->event_info.scan_done.status != ESP_OK) {
		ESP_LOGI(TAG, "Scan failed. Event status: 0x%x", event->event_info.scan_done.status);
		goto err_out;
	}
	...
	if (num_aps > MAX_NUM_APS) {
		ESP_LOGI(TAG, "Limiting AP records to %d (Actually found %d)", MAX_NUM_APS, num_aps);
		num_aps = MAX_NUM_APS;
	}
...
static esp_err_t wifi_start_scan(void) {
	...
	if (mode != WIFI_MODE_APSTA && mode != WIFI_MODE_STA) {
		ESP_LOGI(TAG, "[%s] Invalid WiFi mode for scanning.", __FUNCTION__);
		...
		if (result != ESP_OK) {
			// result = ESP_FAIL;
			ESP_LOGE(TAG, "[%s] Cannot set mode to APSTA or STA.", __FUNCTION__);
			goto err_out;
		} else {
			ESP_LOGI(TAG, "[%s] STA or APSTA mode set.", __FUNCTION__);
		}
	}
...
static void set_wifi_cfg(struct wifi_cfg *cfg) {
	...
	if (cfg->mode == WIFI_MODE_NULL) {
		...
		if (result != ESP_OK) {
			ESP_LOGE(TAG, "[%s] esp_wifi_stop(): %d %s", __FUNCTION__, result, esp_err_to_name(result));
		}
	}
	...
	if (result != ESP_OK) {
		ESP_LOGE(TAG, "[%s] esp_wifi_set_mode(): %d %s", __FUNCTION__, result, esp_err_to_name(result));
	}
	...
	if (cfg->mode == WIFI_MODE_APSTA || cfg->mode == WIFI_MODE_AP) {
		...
		if (result != ESP_OK) {
			ESP_LOGE(TAG, "[%s] esp_wifi_set_config() AP: %d %s", __FUNCTION__, result, esp_err_to_name(result));
		}
	}
	...
	if (cfg->mode == WIFI_MODE_APSTA || cfg->mode == WIFI_MODE_STA) {
		...
		if (result != ESP_OK) {
			ESP_LOGE(TAG, "[%s] esp_wifi_set_config() STA: %d %s", __FUNCTION__, result, esp_err_to_name(result));
		}
	}

	result = esp_wifi_start();
	if (result != ESP_OK) {
		ESP_LOGE(TAG, "[%s] esp_wifi_start(): %d %s", __FUNCTION__, result, esp_err_to_name(result));
	}

	if (cfg->connect && (cfg->mode == WIFI_MODE_STA || cfg->mode == WIFI_MODE_APSTA)) {
		result = esp_wifi_connect();
		if (result != ESP_OK) {
			ESP_LOGE(TAG, "[%s] esp_wifi_connect(): %d %s", __FUNCTION__, result, esp_err_to_name(result));
		}
	}
}
...
static void handle_config_timer(TimerHandle_t timer) {
	...
	if (xSemaphoreTake(cfg_state.lock, 0) != pdTRUE) {
		if (xTimerChangePeriod(config_timer, CFG_DELAY, CFG_DELAY) != pdPASS) {
			ESP_LOGE(TAG, "[%s] Failure to get config lock and change timer.", __FUNCTION__);
		}
		return;
	}

	ESP_LOGD(TAG, "[%s] Called. State: %s", __FUNCTION__, state_names[cfg_state.state]);
	...
	switch (cfg_state.state) {
		case cfg_state_wps_start:
			...
			if (result != ESP_OK) {
				ESP_LOGE(
					TAG, "[%s] esp_wifi_wps_enable() failed: %d %s", __FUNCTION__, result, esp_err_to_name(result));
			...
			if (result != ESP_OK) {
				ESP_LOGE(TAG, "[%s] esp_wifi_wps_start() failed: %d %s", __FUNCTION__, result, esp_err_to_name(result));
				cfg_state.state = cfg_state_fallback;
				delay = CFG_DELAY;
			}
		...
		case cfg_state_wps_active:
			...
			if (events & BIT_WPS_SUCCESS) {
				...
				if (result != ESP_OK) {
					ESP_LOGE(TAG, "[%s] wifi wps disable: %d %s", __FUNCTION__, result, esp_err_to_name(result));
				}
				...
				ESP_LOGI(TAG, "[%s] WPS failed, restoring saved config.", __FUNCTION__);
		...
		case cfg_state_update:
			...
			if (cfg_state.new.mode == WIFI_MODE_AP || cfg_state.new.mode == WIFI_MODE_NULL || !cfg_state.new.connect) {
				...
				ESP_LOGI(TAG, "[%s] Config state set to cfg_state_connecting. ", __FUNCTION__);
				delay = CFG_TICKS;
			}
			break;
		...
		case cfg_state_connecting:
			...
			if (connected) {
				...
				ESP_LOGI(TAG, "[%s] Config state set to cfg_state_connected. ", __FUNCTION__);
				...
				\* saved configuration.                                          */
				cfg_state.state = cfg_state_fallback;
				ESP_LOGI(TAG, "[%s] Timed out: Config state set to cfg_state_fallback. ", __FUNCTION__);
				delay = CFG_DELAY;
			} else {
				/* Twiddle our thumbs and keep waiting for the connection.  */
				ESP_LOGI(TAG, "[%s] Waiting.... ", __FUNCTION__);
				delay = CFG_TICKS;
			}
			break;
		...
		case cfg_state_failed:
			ESP_LOGI(TAG, "[%s] Failed.", __FUNCTION__);
			break;
	}
...
err_out:
	...
	ESP_LOGD(TAG, "[%s] Leaving. State: %s delay: %d", __FUNCTION__, state_names[cfg_state.state], delay);
...
void cgiWifiEventCb(system_event_t *event) {
	ESP_LOGD(TAG, "[%s] Received %s.", __FUNCTION__, event_names[event->event_id]);
...
static esp_err_t update_wifi(struct wifi_cfg_state *cfg, struct wifi_cfg *new) {
	...
	if (result != ESP_OK) {
		ESP_LOGI(TAG, "[%s] Error fetching current WiFi config.", __FUNCTION__);
		goto err_out;
	}
	...
	connected = sta_connected();
	if (!connected) {
		ESP_LOGI(TAG, "[%s] sta_connected() false", __FUNCTION__);
...
CgiStatus cgiWiFiConnect(HttpdConnData *connData) {
	...
	ESP_LOGI(TAG, "cgiWiFiConnect is called");
	ESP_LOGI(TAG, "connData->requestType = %d", connData->requestType);
	ESP_LOGI(TAG, "connData->url = %s", connData->url);
	if (connData->post.buff == NULL)
		ESP_LOGI(TAG, "connData->post.buff is NULL");
	else
		ESP_LOGI(TAG, "connData->post.buff = %s", connData->post.buff);
	if (connData->getArgs == NULL)
		ESP_LOGI(TAG, "connData->getArgs is NULL");
	else
		ESP_LOGI(TAG, "connData->getArgs is %s", connData->getArgs);

	sta = &(cfg.sta.sta);
	len = httpdFindArg(connData->post.buff, "essid", (char *)&(sta->ssid), sizeof(sta->ssid));
	if (len <= 1) {
		len = httpdFindArg(connData->getArgs, "essid", (char *)&(sta->ssid), sizeof(sta->ssid));
		if (len <= 1) {
	...
	if (len <= 1) {
		len = httpdFindArg(connData->getArgs, "passwd", (char *)&(sta->password), sizeof(sta->password));
		if (len <= 1) {
...
CgiStatus cgiWiFiSetMode(HttpdConnData *connData) {
	...
	if (len != 0) {
		...
		result = get_wifi_cfg(&cfg);
		if (result != ESP_OK) {
			ESP_LOGE(TAG, "[%s] Error fetching current WiFi config.", __FUNCTION__);
			goto err_out;
		}
		...
		if (mode == WIFI_MODE_STA && !sta_connected()) {
			ESP_LOGE(TAG,
					 "[%s] No connection to AP, not switching to "
					 "client-only mode.",
					 __FUNCTION__);
			goto err_out;
		}
		...
		ESP_LOGI(TAG,
				 "[%s] Switching to WiFi mode %s",
				 __FUNCTION__,
				 mode == WIFI_MODE_AP
					 ? "SoftAP"
					 : mode == WIFI_MODE_APSTA ? "STA+AP" : mode == WIFI_MODE_STA ? "Client" : "Disabled");
	...
	#else
		ESP_LOGI(TAG,
				 "[%s] Demo mode, not switching to WiFi mode %s",
				 __FUNCTION__,
				 mode == WIFI_MODE_AP
					 ? "SoftAP"
					 : mode == WIFI_MODE_APSTA ? "STA+AP" : mode == WIFI_MODE_STA ? "Client" : "Disabled");
...
CgiStatus cgiWiFiAPSettings(HttpdConnData *connData) {
	...
	if (len > 0) {
		...
		if (errno != 0 || chan < 1 || chan > 15) {
			ESP_LOGW(TAG, "[%s] Not setting invalid channel %s", __FUNCTION__, buff);
		} else {
			has_arg_chan = true;
		}
	}
	...
	if (len > 0) {
		...
		if (n == 1) {
			...
			ESP_LOGW(TAG, "[%s] Not setting invalid ssid %s", __FUNCTION__, buff);
		}
	}
	...
	if (len > 0) {
		...
		if (n == 1) {
			...
			ESP_LOGW(TAG, "[%s] Not setting invalid pass %s", __FUNCTION__, buff);
		}
	}
	...
	if (has_arg_chan || has_arg_ssid || has_arg_pass) {
		...
		if (has_arg_ssid) {
			...
			cfg.ap.ap.ssid_len = 0; // if ssid_len==0, check the SSID until there is a termination character; otherwise,
									// set the SSID length according to softap_config.ssid_len.
	...
	if (!strcmp(token, "WiFiMode")) {
		...
		switch (mode) {
			case WIFI_MODE_AP:
				...
				snprintf(buff,
						 sizeof(buff) - 1,
						 "<p><button onclick=\"location.href='setmode.cgi?mode=%d'\">Go to <b>AP+STA</b> mode</button> "
			...
			case WIFI_MODE_APSTA:
				snprintf(buff,
						 sizeof(buff) - 1,
						 "<p><button onclick=\"location.href='setmode.cgi?mode=%d'\">Go to standalone <b>AP</b> "
						 "mode</button> (Access Point Only)</p>",
						 WIFI_MODE_AP);
				...
				if (sta_connected()) {
					snprintf(buff + strlen(buff),
							 ...
							 "<p><button onclick=\"location.href='setmode.cgi?mode=%d'\">Go to standalone <b>STA</b> "
							 "mode</button> (Client Only)</p>",
							 WIFI_MODE_STA);
				}
				break;
			case WIFI_MODE_STA:
